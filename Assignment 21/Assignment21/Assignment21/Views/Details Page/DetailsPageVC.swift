...
final class DetailsPageVC: UIViewController {
    ...
    private let bannerImageButton: UIButton = {
        let button = UIButton()
        button.setImage(UIImage(named: "spiderManB"), for: .normal)
    ...
    private func goPosterPage() {
        navigationController?.pushViewController(PosterPageVC(bannerName: detailsPageViewModel.bannerName, title: detailsPageViewModel.title), animated: true)
    }
